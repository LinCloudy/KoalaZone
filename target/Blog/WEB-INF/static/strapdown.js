...
(function() {
		...
		while (src) {
			...
			if (cap = this.rules.list.exec(src)) {
				...
				for (; i < l; i++) {
					...
					if (~item.indexOf('\n ')) {
						space -= item.length;
						item = !this.options.pedantic ? item.replace(
	...
	InlineLexer.prototype.output = function(src) {
		...
		while (src) {
			...
			if (cap = this.rules.autolink.exec(src)) {
				src = src.substring(cap[0].length);
				if (cap[2] === '@') {
					text = cap[1][6] === ':' ? this.mangle(cap[1].substring(7))
							: this.mangle(cap[1]);
	...
	Parser.prototype.tok = function() {
		...
		case 'code': {
			if (this.options.highlight) {
				var code = this.options.highlight(this.token.text,
						this.token.lang);
		...
		case 'table': {
			...
			for (i = 0; i < this.token.header.length; i++) {
				...
				body += this.token.align[i] ? '<th align="'
						+ this.token.align[i] + '">' + heading + '</th>\n'
						: '<th>' + heading + '</th>\n';
			}
			...
			for (i = 0; i < this.token.cells.length; i++) {
				...
				for (j = 0; j < row.length; j++) {
					...
					body += this.token.align[j] ? '<td align="'
							+ this.token.align[j] + '">' + cell + '</td>\n'
							: '<td>' + cell + '</td>\n';
				}
		...
		case 'list_item_start': {
			...
			while (this.next().type !== 'list_item_end') {
				body += this.token.type === 'text' ? this.parseText() : this
						.tok();
			}
		...
		case 'html': {
			return !this.token.pre && !this.options.pedantic ? this.inline
					.output(this.token.text) : this.token.text;
		}
	...
	function escape(html, encode) {
		return html.replace(!encode ? /&(?!#?\w+;)/g : /&/g, '&amp;').replace(
...
(function() {
	...
	function childContentWrapper(element) {
		...
		for (var c = element.firstChild; c; c = c.nextSibling) {
			var type = c.nodeType;
			wrapper = (type === 1) ? (wrapper ? element : c)
					: (type === 3) ? (notWs.test(c.nodeValue) ? element
							: wrapper) : wrapper;
		}
	...
	function sourceDecorator(options) {
		...
		fallthroughStylePatterns.push([ PR_LITERAL, /^@[a-z_$][a-z_$@0-9]*/i,
				...
				PR_PLAIN, /^[a-z_$][a-z_$@0-9]*/i, null ], [ PR_LITERAL,
				new RegExp('^(?:'
				...
				+ '[a-z]*', 'i'), null, '0123456789' ], [ PR_PLAIN,
				/^\\[\s\S]?/, null ], [ PR_PUNCTUATION, punctuation, null ]);
	...
	function numberLines(node, opt_startLineNum, isPreformatted) {
		...
		function breakAfter(lineEndNode) {
			...
			for (var parent; (parent = copiedListItem.parentNode)
					&& parent.nodeType === 1;) {
			...
			while (spanIndex < nSpans) {
				...
				if (textNode.nodeType !== 1
					...
					if (sourceIndex < spanEnd) {
						spans[spanIndex + 1] = textNode = document
								.createTextNode(source.substring(end, spanEnd));
				...
				if (prettyPrintRe.test(className)
					...
					if (!nested) {
						...
						if (preformattedTagNameRe.test(cs.tagName)) {
							...
							var whitespace = (currentStyle ? currentStyle['whiteSpace']
									: (document.defaultView && document.defaultView.getComputedStyle) ? document.defaultView
						...
						var lineNums = cs.className
								.match(/\blinenums\b(?::(\d+))?/);
						lineNums = lineNums ? lineNums[1] && lineNums[1].length ? +lineNums[1]
								: true
								: false;
						if (lineNums) {
